...
import {
  listGatewayAuthzAllowEventsPage,
  summarizeGatewayAuthzAllowEvents,
...
import {
  ...
  validateAuthzAllowListParams,
  validateAuthzAllowSummaryParams,
...
export const authzHandlers: GatewayRequestHandlers = {
  "authz.allow.list": ({ respond, params }) => {
    if (!validateAuthzAllowListParams(params ?? {})) {
      respond(
        false,
        undefined,
        errorShape(
          ErrorCodes.INVALID_REQUEST,
          `invalid authz.allow.list params: ${formatValidationErrors(validateAuthzAllowListParams.errors)}`,
        ),
      );
      return;
    }
    ...
    const page = listGatewayAuthzAllowEventsPage({
      limit: raw.limit,
  ...
  "authz.allow.summary": ({ respond, params }) => {
    if (!validateAuthzAllowSummaryParams(params ?? {})) {
      respond(
        false,
        undefined,
        errorShape(
          ErrorCodes.INVALID_REQUEST,
          `invalid authz.allow.summary params: ${formatValidationErrors(validateAuthzAllowSummaryParams.errors)}`,
        ),
      );
      return;
    }
    ...
    const summary = summarizeGatewayAuthzAllowEvents({
      topN: raw.topN,
